...
class Validatr {
  constructor(form) {
    this.forms = form || document.querySelectorAll("[data-validatr-form]");
    ...
    this.rules = {
      ...
      min: {
        test: (value, input) => {
          return (
            value.length >= parseInt(input.getAttribute("data-validatr-min"))
          );
        },
        message: (input) => {
      ...
      max: {
        test: (value, input) => {
          return (
            value.length <= parseInt(input.getAttribute("data-validatr-max"))
          );
        },
        message: (input) => {
      ...
      same: {
        test: (value, input) => {
          const otherInput = input.getAttribute("data-validatr-same");
      ...
      dateBefore: {
        test: (value, input) => {
          const otherValue = document.querySelector(
            `[name="${input.getAttribute("data-validatr-dateBefore")}"]`
          ).value;

          return Date.parse(value) < Date.parse(otherValue);
        },
        message: (input) => {
      ...
      dateAfter: {
        test: (value, input) => {
          const otherValue = document.querySelector(
            `[name="${input.getAttribute("data-validatr-dateAfter")}"]`
          ).value;
      ...
      between: {
        test: (value, input) => {
          const values = input.getAttribute("data-validatr-between").split(",");
        ...
        message: (input) => {
          let value = input.getAttribute("data-validatr-between");
      ...
      different: {
        test: (value, input) => {
          const otherValue = document.querySelector(
            `[name="${input.getAttribute("data-validatr-different")}"]`
          ).value;
      ...
      form.addEventListener("submit", (e) => {
        ...
        if (!this.validateForm(form)) {
          ...
          document.dispatchEvent(
            new Event("validatr:submitting", { detail: form })
          );
          ...
          document.dispatchEvent(
            new Event("validatr:submitted", { detail: form })
          );
        }
      });
  ...
  validateInput(input) {
    ...
    const errors = Object.keys(this.rules)
      .filter(
        (rule) =>
          input.hasAttribute(`data-validatr-${rule}`) &&
    ...
    if (isGrouped) {
      ...
      const groupErrors = Object.keys(this.rules)
        .filter(
          (rule) =>
            group[0].hasAttribute(`data-validatr-${rule}`) &&
  ...
  getMessage(input, rule) {
    const customMessage = input.getAttribute(`data-validatr-${rule}-message`);
  ...
  formHasErrors = (form) => {
    return form.querySelectorAll(".validatr-invalid").length;
  };

  displayErrors(input, error) {
    this.removeErrors(input);
    input.classList.add("validatr-invalid");
  ...
  createErrorContainer() {
    ...
    container.classList.add("validatr-invalid-feedback");
  ...
  createErrorElement() {
    ...
    error.classList.add("validatr-error-message");
  ...
  removeErrors(input) {
    ...
    if (input.type === "radio" || input.type === "checkbox") {
      const group = input
        .closest("fieldset")
        .querySelector(".validatr-invalid-feedback");

      if (group) {
        input.classList.remove("validatr-invalid");
        group.remove();
      }
    } else {
      input.classList.remove("validatr-invalid");
